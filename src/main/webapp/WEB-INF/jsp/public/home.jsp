...
<main class="main-content">
  ...
  <c:forEach var="category" items="${bannerCategories}">
    <c:if test="${category.enabled}">
      ...
      <c:if test="${categoryKey == 'top_banner'}">
        <c:set var="topBannerCategory" value="${category}" scope="request"/>
        <c:set var="topBanners" value="${banners}" scope="request"/>
      </c:if>
      <c:if test="${categoryKey == 'middle_banner'}">
        <c:set var="middleBannerCategory" value="${category}" scope="request"/>
        <c:set var="middleBanners" value="${banners}" scope="request"/>
      </c:if>
      <c:if test="${categoryKey == 'bottom_banner'}">
        <c:set var="bottomBannerCategory" value="${category}" scope="request"/>
        <c:set var="bottomBanners" value="${banners}" scope="request"/>
      </c:if>
      <c:if test="${categoryKey == 'side_banner'}">
        <c:set var="sideBannerCategory" value="${category}" scope="request"/>
        <c:set var="sideBanners" value="${banners}" scope="request"/>
      </c:if>
      <c:if test="${categoryKey == 'popup_banner'}">
        <c:set var="popupBannerCategory" value="${category}" scope="request"/>
        <c:set var="popupBanners" value="${banners}" scope="request"/>
      </c:if>
    </c:if>
  </c:forEach>
  ...
  <c:if test="${not empty topBanners}">
    <section class="hero-banner">
            ...
            <c:choose>
              ...
              <c:when test="${b.type.name() == 'YOUTUBE'}">
                <div class="video-wrapper">
                  <iframe src="${b.url}?autoplay=1&mute=1&loop=1&controls=0&rel=0&modestbranding=1&playlist=${fn:substringAfter(b.url, 'embed/')}"
                          title="${b.title}" frameborder="0"
                          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                          allowfullscreen></iframe>
                </div>
              </c:when>
              <c:otherwise>
                <video src="${b.url}" autoplay muted loop playsinline class="banner-video"></video>
              </c:otherwise>
            </c:choose>
          </div>
        </c:forEach>
        <div class="hero-overlay"></div>
        <c:if test="${topBanners.size() > 1}">
      ...
      <div class="hero-content">
        <span class="subtitle">Global Talent Development</span>
  ...
  <%-- ========== 2. 중간 배너 (프로그램) ========== --%>
  <c:if test="${not empty middleBanners}">
      ...
      <div class="container">
        <div class="section-header">
          ...
          <h2 class="section-title">
            <c:choose>
              <c:when test="${not empty middleBannerCategory.name}"><c:out value="${middleBannerCategory.name}"/></c:when>
              <c:otherwise>${lang == 'ko' ? '중간 배너' : 'Middle Banner'}</c:otherwise>
            </c:choose>
          </h2>
          <c:if test="${not empty middleBannerCategory.description}">
            <p class="section-desc"><c:out value="${middleBannerCategory.description}"/></p>
          </c:if>
        </div>
        <div class="banner-scroll-container">
          <div class="banner-scroll-track">
            <c:forEach var="b" items="${middleBanners}">
              <div class="banner-scroll-item">
                <c:choose>
                  <c:when test="${b.type.name() == 'IMAGE'}">
                    <a href="${not empty b.linkUrl ? b.linkUrl : '#'}" class="banner-card-link">
                      <div class="banner-card-image">
                        <img src="${b.url}" alt="${b.title}"/>
                      </div>
                      <c:if test="${not empty b.title}">
                        <div class="banner-card-title"><c:out value="${b.title}"/></div>
                      </c:if>
                    </a>
                  </c:when>
                  <c:when test="${b.type.name() == 'YOUTUBE'}">
                    <div class="banner-card-video">
                  ...
                  <c:otherwise>
                    <div class="banner-card-video">
                      <video src="${b.url}" controls playsinline></video>
                    </div>
                    <c:if test="${not empty b.title}">
                      <div class="banner-card-title"><c:out value="${b.title}"/></div>
                    </c:if>
  ...
  <c:if test="${not empty bottomBanners}">
    <section class="section bottom-banner-section fade-in">
      <div class="container">
        <c:if test="${not empty bottomBannerCategory.name}">
          <div class="section-header">
            <h2 class="section-title"><c:out value="${bottomBannerCategory.name}"/></h2>
            <c:if test="${not empty bottomBannerCategory.description}">
              <p class="section-desc"><c:out value="${bottomBannerCategory.description}"/></p>
            </c:if>
          </div>
        </c:if>
        <div class="banner-scroll-container bottom-scroll">
          <div class="banner-scroll-track">
            <c:forEach var="b" items="${bottomBanners}">
              <div class="banner-scroll-item">
                <c:choose>
                  <c:when test="${b.type.name() == 'IMAGE'}">
                    <a href="${not empty b.linkUrl ? b.linkUrl : '#'}" class="banner-card-link">
  ...
  <section class="section features-section fade-in">
    <div class="container">
      ...
      <!-- 모바일: 스크롤 슬라이더 -->
      <div class="features-scroll-container mobile-only">
  ...
  <%-- ========== 사이드 배너 ========== --%>
  <c:if test="${not empty sideBanners}">
    <div class="side-banner-container" id="sideBanner">
      <c:forEach var="b" items="${sideBanners}" varStatus="status">
        <c:if test="${status.index < 3}">
          <div class="side-banner-item">
            <c:choose>
              <c:when test="${b.type.name() == 'IMAGE'}">
                <a href="${not empty b.linkUrl ? b.linkUrl : '#'}" target="_blank">
                  <img src="${b.url}" alt="${b.title}"/>
                </a>
              </c:when>
              <c:otherwise>
  ...
  <c:if test="${not empty popupBanners}">
    <div id="popup-banner-data" style="display:none;"
         data-banners='<c:forEach var="b" items="${popupBanners}" varStatus="s">{"id":${b.id},"title":"<c:out value="${b.title}"/>","url":"${b.url}","linkUrl":"${b.linkUrl}","type":"${b.type.name()}"}<c:if test="${!s.last}">,</c:if></c:forEach>'
         data-max-count="${popupBannerCategory.popupMaxCount}"
         data-duration="${popupBannerCategory.popupDuration}"
         data-lang="${lang}">
    </div>
  </c:if>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ===== Hero Banner Slider =====
  const slider = document.querySelector('.hero-banner .banner-slider');
  if (slider) {
    ...
    dots.forEach((dot, i) => dot.addEventListener('click', function() { stopAutoSlide(); showSlide(i); startAutoSlide(); }));
  ...
  function openModal() { inquiryModal.classList.add('active'); document.body.style.overflow = 'hidden'; }
  function closeModal() { inquiryModal.classList.remove('active'); document.body.style.overflow = ''; }
  ...
  function closeResultModal() { resultModal.classList.remove('active'); document.body.style.overflow = ''; }
  ...
  function submitInquiry(form, successCallback) {
    ...
    .then(result => {
      ...
      if (result.ok) {
        ...
        showResultModal(true, lang === 'ko' ? '상담 신청 완료' : 'Inquiry Submitted', lang === 'ko' ? '상담 신청이 완료되었습니다.\n빠른 시일 내에 연락드리겠습니다.' : 'Your inquiry has been submitted.\nWe will contact you soon.');
      } else {
        showResultModal(false, lang === 'ko' ? '오류 발생' : 'Error', lang === 'ko' ? '오류가 발생했습니다. 다시 시도해주세요.' : 'An error occurred. Please try again.');
      }
    })
    .catch(error => {
  ...
  if (inquiryForm) inquiryForm.addEventListener('submit', function(e) { e.preventDefault(); submitInquiry(this); });

  const modalInquiryForm = document.getElementById('modalInquiryForm');
  if (modalInquiryForm) modalInquiryForm.addEventListener('submit', function(e) { e.preventDefault(); submitInquiry(this, closeModal); });
    ...
    if (bannersAttr) {
      try {
        ...
        if (document.cookie.indexOf(cookieName) === -1 && banners.length > 0) {
          ...
          const popupBanners = banners.slice(0, maxCount);
          ...
          function showPopup(index) {
            if (index >= popupBanners.length) { overlay.remove(); return; }
            const banner = popupBanners[index];
            const popup = document.createElement('div');
            popup.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:10000;background:#fff;box-shadow:0 4px 20px rgba(0,0,0,0.3);border-radius:12px;overflow:hidden;' + (isMobile ? 'width:90vw;max-width:380px;' : 'max-width:500px;width:90vw;');

            let content = '';
            if (banner.type === 'IMAGE') {
              content = '<a href="' + (banner.linkUrl || '#') + '"><img src="' + banner.url + '" alt="' + banner.title + '" style="width:100%;max-height:' + (isMobile ? '55vh' : '65vh') + ';object-fit:contain;display:block;"/></a>';
            ...
            const closeBtnText = popupLang === 'ko' ? '닫기' : 'Close';
            const counterText = popupBanners.length > 1 ? '<span style="font-size:11px;color:#999;">' + (index + 1) + ' / ' + popupBanners.length + '</span>' : '';

            popup.innerHTML = content + '<div style="padding:12px 14px;border-top:1px solid #eee;display:flex;justify-content:space-between;align-items:center;gap:10px;">' + counterText + '<div style="display:flex;align-items:center;gap:12px;margin-left:auto;"><label style="cursor:pointer;font-size:12px;color:#666;display:flex;align-items:center;white-space:nowrap;"><input type="checkbox" class="popup-today-close" style="margin-right:5px;width:16px;height:16px;"/> ' + closeText + '</label><button class="popup-close" style="padding:10px 18px;cursor:pointer;background:#2563eb;color:white;border:none;border-radius:8px;font-weight:600;font-size:14px;">' + closeBtnText + '</button></div></div>';
            document.body.appendChild(popup);

            function closePopup() {
              if (popup.querySelector('.popup-today-close').checked) {
                document.cookie = cookieName + '=1;path=/;max-age=' + (duration * 3600);
                overlay.remove(); popup.remove(); return;
              }
              popup.remove();
              showPopup(index + 1);
            }
          ...
          overlay.addEventListener('click', function() { document.querySelectorAll('.popup-banner').forEach(p => p.remove()); overlay.remove(); });
          showPopup(0);
        }
      } catch (e) { console.error('Popup banner parse error:', e); }
    }
  }
});
</script>
